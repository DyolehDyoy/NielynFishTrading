 ...
 */
 ...
 function format_currency_peso($amount, $currencyCode) {
    // Add more formatting options if needed
    return '₱' . number_format($amount, 2, '.', ','); // Peso symbol and 2 decimal places
	...
	// Check if the currency code is for Philippine Peso
    if ($curr == 'PHP') {
        $number = formatphp($floatcurr);
    } else {
        $number = number_format($floatcurr, $currencies[$curr][1], $currencies[$curr][2], $currencies[$curr][3]);

        // Adding the symbol in the back or normally in front
